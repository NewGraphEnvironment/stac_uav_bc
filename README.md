...
| `stac_create_item.py` | Create STAC items + update collection.json for new COGs (additive-only) |
| `web.R` | Web/viewer utilities |
| `config/` | Server docs + registration scripts (`stac_register_item.sh`, `stac_register_collection.sh`) — see the [add-imagery recipe](scripts/config/README.md) |
